(function(angular) { 'use strict';
	...
	*/
	angular.module('trello.service', []).
	factory('TrelloService', ['$rootScope', '$http','$q', '$window', function($rootScope,$http, $q, $window) {
		...
		var service = {
			...
			accumulateBoardInfo: function() {
				...
				$http.get(baseUrl + '/accumulate').then(function(response) {
					d.resolve(response);
			...
			*/			
			period: function(boardid) {
				...
				return $http.get(baseUrl + '/period/' + boardid).then(function(period) {
					//d.resolve(period);
					return period;
				});
				// .catch(function(e) {
			...
			series: function(boardid) {
				...
				return $http.get(baseUrl + '/series/' + boardid).then(function(series) {
					return series;
					//d.resolve(series);
				});
				// .catch(function(e) {					
